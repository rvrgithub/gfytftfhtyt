...
const generateEmployeeId = async (req, res) => {
    ...
    try {
        ...
        if (!isNumberstring(employee)) { return res.status(400).send({ status: false, message: "Employer Id Consists Number And String." }) }
...
const userStatus=async(req,res)=>{
    ...
    console.log(userName,empStatus)
...
const epmpoyeeIdList=async(req,res)=>{
    try {
         let response=await EmployeeId.find()
         res.status(200).json({status:true,message:response})
